...
vector<int> InsertionSort(vector<int> vec) //OK
{
	int i,j,temp;
	int swaps=0;
...
vector<int> SelectionSort(vector<int> vec) //OK
{
	int i,j,temp;
	int swaps=0;
...
vector<int> QuickSort(vector<int> vec, int left, int right) //OK
{ 
...
void merge(vector<int>& vec, int start, int mid, int end) //utility function for MergeSort //OK
{
	// create a temp array
...
vector<int> MergeSort(vector<int> &vec, int left, int right) //OK
{
...
void heapify(vector<int>& vec, int n, int i) //utility function for HeapSort  //OK
{
	int temp;
	int largest=i;
	int left = 2*i +1;
...
vector<int> HeapSort(vector<int> vec, int n) //OK
{
	int temp;
...
int BinarySearch(vector<int> vec, int x) //OK
{ 
	int bottom=0;
	int top = (vec.size() - 1);
	int middle;
...
int InterpolationSearch(vector<int> vec, int x) //OK
{ 
	start=clock(); //beginning of time counting
...
int BinaryInterpolationSearch(vector<int> vec, int x) //OK
{ 
	int flag=0;
	bool whileFlag = true;
	int left = 0;
...
//////// UTILITY FUNCTIONS ///////////
void printArray(int arr[], int size) //OK
{ 
    int i; 
    for (i=0; i < size; i++) 
    cout<<arr[i]<<endl; 
}
void printVector(vector<int> vec) //OK
{
	string yesOrNo;
